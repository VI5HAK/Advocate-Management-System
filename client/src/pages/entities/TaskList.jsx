...
export function TaskList() {
  ...
  const fetchTasks = async (searchTerm = "") => {
    ...
    setError("");
    try {
      const res = await entityService.getTasks(searchTerm);
      setTasks(res.data || []);
    } catch (err) {
      ...
      <header className="flex items-center justify-between gap-3 sm:gap-4">
        <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2 sm:gap-3 min-w-0">
          <div className="p-1.5 sm:p-2 rounded-xl bg-indigo-50 text-indigo-600 shrink-0">
            <CheckSquare className="h-6 w-6 sm:h-8 sm:w-8 shrink-0" />
          </div>
          <span className="truncate">Task Management</span>
          <HelpButton title="Task Management" />
        </h1>
        {!isAdvocate && (
          <CreateButton
      ...
      <form className="flex gap-3" onSubmit={handleSearchSubmit}>
        <div className="relative flex-1 flex">
          ...
          <input
            ...
            placeholder="Search tasks by name, client, advocate, case, category, status..."
            value={search}
                            ...
                            <div className="flex items-center justify-between border-b border-indigo-100/50 pb-2">
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  ...
                                  className="px-3 py-1.5 rounded-xl bg-white border border-slate-200 hover:border-indigo-300 text-xs font-bold text-indigo-700 shadow-sm hover:shadow transition-all inline-flex items-center gap-1.5 cursor-pointer"
                                >
